...
RSpec.describe Warehouse do
  ...
  context "#portfolio" do
    it "returns array of products when initialied with array of products" do
      expect(Warehouse.new([@product1, @product2, @product3]).portfolio).to eql([@product1, @product2, @product3])
    end
  ...
  context "#add" do
    let(:warehouse_init) {Warehouse.new([@product1])}
  ...
  context "#remove" do
    ...
    it "raises error when wrong number of arguments" do
      expect{
        warehouse_init.remove(1)
      }.to raise_error(ArgumentError)
    end

    it "raises error when not positive quantity" do
      expect{
        warehouse_init.remove(1, -2)
      }.to raise_error(ArgumentError)
    end

    it "raises error when quantity not number" do
      expect{
        warehouse_init.remove(1, "start")
      }.to raise_error(ArgumentError)
    end
    ...
    it "raises error when product_id not number" do
      expect{
        warehouse_init.remove(nil, 2)
      }.to raise_error(ArgumentError)
    end

    it "raises error when product_id is not in portfolio" do
      expect{
        warehouse_init.remove(7, 2)
      }.to raise_error(ArgumentError)
    end

    it "operates on integer" do
      expect{
        warehouse_init.remove(1, 2.3)
      }.to raise_error(ArgumentError)
    end
  end
  ...
  context "#is_in_portfolio?" do
    ...
    it "returns true when product with given id is in portfolio" do
      expect(warehouse_init.is_in_portfolio?(1)).to be true
    end

    it "returns false when product with given id is not in portfolio" do
      expect(warehouse_init.is_in_portfolio?(2)).to be false
    end

    it "raises error if wrong argument" do
      expect{
        warehouse_init.is_in_portfolio?(nil)
      }.to raise_error(ArgumentError)
    end
  end
  ...
  context "#to_s" do
    it "properly converts to string" do
      warehouse = Warehouse.new([@product1])
      warehouse.add(1, 10)
